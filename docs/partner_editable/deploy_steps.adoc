...
. On the *Specify stack details* page, change the stack name if needed. Review the parameters for the template. Provide values for the parameters that require input. For all other parameters, review the default settings and customize them as necessary. For details on each parameter, see the below link for parametert section.
* link:#_launch_into_a_new_vpc[Parameters for deploying SageMaker with guardrails into a new VPC]
* link:#_launch_into_an_existing_vpc[Parameters for deploying SageMaker with guardrails into an existing VPC]
+
