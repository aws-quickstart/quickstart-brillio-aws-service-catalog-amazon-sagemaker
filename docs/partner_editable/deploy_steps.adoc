...
[cols=2*]
|===
^|https://fwd.aws/Dg76J[Deploy {partner-product-name} into a new VPC on AWS^]
^|https://fwd.aws/qnrXg[View template^]

^|https://fwd.aws/RjGV8[Deploy {partner-product-name} into an existing VPC on AWS^]
^|https://fwd.aws/4wnvp[View template^]
|===

// Adapt the following warning to your Quick Start.
WARNING: If you deploy this Quick Start in an existing VPC, ensure that your VPC is private with no internet gateway attachment. This deployment creates two subnets in an existing VPC. Also, ensure that the Domain Name System (DNS) hostname and DNS support is enabled on existing VPC attributes. Otherwise, the QuickStart enables it during the provisioning process.
...
. On the *Specify stack details* page, change the stack name if needed. Review the parameters for the template. Provide values for the parameters that require input. For all other parameters, review the default settings and customize them as necessary. See the link:#_parameter_reference[Parameter reference] for more information. 
+
